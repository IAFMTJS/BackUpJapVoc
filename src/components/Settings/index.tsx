...
import { saveSettings, loadSettings, resetSettings as resetSettingsStorage } from '../../utils/settingsStorage';
...
const Settings: React.FC = () => {
  ...
  useEffect(() => {
    const loadSavedSettings = async () => {
      try {
        ...
        if (savedSettings.notificationSettings) {
          setNotificationSettings(savedSettings.notificationSettings);
        }
        if (savedSettings.languageSettings) {
          setLanguageSettings(savedSettings.languageSettings);
        }
        if (savedSettings.themeSettings) {
          setThemeSettings(savedSettings.themeSettings);
        }
        if (savedSettings.studySettings) {
          setStudySettings(savedSettings.studySettings);
        }
        if (savedSettings.securitySettings) {
          setSecuritySettings(savedSettings.securitySettings);
        }
      } catch (error) {
  ...
  const handleSaveSettings = async (section: string) => {
    try {
      ...
      switch (section) {
        case 'notifications':
          settingsToSave.notificationSettings = notificationSettings;
          break;
        case 'language':
          settingsToSave.languageSettings = languageSettings;
          break;
        case 'appearance':
          settingsToSave.themeSettings = themeSettings;
          break;
        case 'study':
          settingsToSave.studySettings = studySettings;
          break;
        case 'security':
          settingsToSave.securitySettings = securitySettings;
          break;
        case 'all':
          settingsToSave.notificationSettings = notificationSettings;
          settingsToSave.languageSettings = languageSettings;
      ...
      action: async () => {
        try {
          if (section === 'all') {
            ...
            switch (section) {
              case 'notifications':
                settingsToReset.notificationSettings = {
                  dailyReminder: true,
                  goalReminders: true,
                  achievementAlerts: true,
                  progressUpdates: true,
                ...
                setNotificationSettings(settingsToReset.notificationSettings);
                break;
              // ... similar cases for other sections ...
            }
            await saveSettings(settingsToReset);
          }
